...
the agent back to it through the `claude` workflow. When the `review` workflow's action cannot
run, because the pull request changes `review.yml` itself, the job says so in a comment and its
check fails; the pre-flight reports stand in.
